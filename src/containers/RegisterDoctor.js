...
const RegisterDoctor = ({isLoggedIn, registerDoctor, specialities,getSpecialities}) => {
    ...
    <div className="register-container">
      <form onSubmit={(e) => {
        ...
        <label htmlFor="experience_year">
          <input id="experience_year" type="experience_year" onChange={(e)=>setExperienceYear(e.target.value)} value={experience_year} placeholder="Experience year" />
        </label>
        <label htmlFor="hospital">
          <input id="hospital" type="hospital" onChange={(e)=>setHospital(e.target.value)} value={hospital} placeholder="Hospital" />
        </label>
      ...
      {(specialities && specialities.length>0)?specialities.map(sp=><option key={sp.id} value={sp.id}>{sp.name}</option>):<option>No Speciality</option>}
        ...
        <p>Not a doctor? <Link to="register">Register as patient</Link></p>
      </form>
    </div>
  );
};
